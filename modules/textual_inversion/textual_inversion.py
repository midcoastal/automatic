...
from modules.modelloader import directory_mtime
from typing import List, Optional, Union
from modules.textual_inversion.loaders import suppress_exceptions
...
def list_embeddings(*dirs):
    ...
    return [*filter(lambda fp: is_ext(fp) and is_not_preview(fp) and os.stat(fp).st_size > 0, directory_files(*dirs))]
...
class EmbeddingDatabase:
    ...
    def load_diffusers_embedding(
        ...
        _debug = debug.prefix(f'load_diffusers_embedding() - ')
        _debug.debug('begin')
        ...
        if shared.sd_model is not None:
            ...
            _debug(f'Detected Model Type: {model_type}')
            try:
                if model_type:
                    unk_token_id = tokenizer.convert_tokens_to_ids(tokenizer.unk_token)
                    _debug.debug(f'tokenizer.convert_tokens_to_ids(tokenizer.unk_token) = {unk_token_id}')
                    for filename, path in filename_paths:
                        ...
                        try:
                            ...
                            _debug.debug(f'Embedding `{name}` Invalid - {e}')
                            skipped_embeddings[name] = embedding
                            continue
                        embeddings_to_load = sorted(embeddings_to_load, key=lambda e: exts.index(os.path.splitext(e.filename)[1].upper()))
                    _debug(f'Pre-Validated {len(embeddings_to_load)} of {len([ e for e in filename_paths])} Embeddings')
                    ...
                    if model_type == 'SD':
                        ...
                        _debug.debug(f'Loaded {len(loaded_filenames)} Embeddings with `pipe.load_textual_inversion()`\n{loaded_filenames}')
                        ...
                        _debug.debug(f'tracking {len(loaded_embeddings)} in `loaded_embeddings`\nlen(embeddings) went from {_len} to {len(embeddings_to_load)} (a diff of {_len-len(embeddings_to_load)})')
                    else:
                        _debug.debug(f'{model_type} Model skipping `pipe.load_textual_inversion()`')
                    tokens_to_add = {}
                    tokenizer_vocab = tokenizer.get_vocab()
                    for embedding in embeddings_to_load:
                        try:
                            name = embedding.name
                            _debug.debug(f'Parsing Embedding: {name}')
                            ...
                            if ext in ['.SAFETENSORS']:
                                with safetensors.torch.safe_open(embedding.filename, framework="pt") as f:
                                    for k in f.keys():
                            ...
                            if 'clip_g' in embeddings_dict:
                                ...
                                embedding_type = 'SD'
                            _debug.debug(f'Embedding Type: {embedding_type}')
                    ...
                    if len(tokens_to_add) > 0:
                        ...
                        _debug(f'added {num_added} (of {len(tokens_to_add)} queued) tokens, tokenizer size: {len(tokenizer)} (was {_tokenizer_len})')
                        if len(tokens_to_add) != num_added:
                            _debug.warning(f'number of queued tokens does not match the number of added tokens: {len(tokens_to_add)} != {num_added} \n{tokens_to_add.keys()}')
                        clip_l.resize_token_embeddings(len(tokenizer))
                        if model_type == 'SD-XL':
                            tokenizer_2.add_tokens([k for k in tokens_to_add.keys()])
                            clip_g.resize_token_embeddings(len(tokenizer))
                        for token, data in tokens_to_add.items():
                            token_id = tokenizer.convert_tokens_to_ids(token)
                            _debug.debug(f'adding {token} (token_id: {token_id}) data')
                            if token_id > unk_token_id:
                                clip_l.get_input_embeddings().weight.data[token_id] = data.clip_l
                                if model_type == 'SD-XL':
                                    clip_g.get_input_embeddings().weight.data[token_id] = data.clip_g
                            else:
                                _debug.error(f"token '{token}' was queued to be added, but does not resolve")
            except Exception as e:
        ...
        for name, embedding in skipped_embeddings.items():
            if loaded_embeddings.get(name, None) == embedding:
                continue
